...
<div class="container my-5" style="max-width: 800px;">
    <h4 class="mb-4 fw-semibold text-dark">Edit Product</h4>
    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            @if($product->image)
                <img src="{{ asset('storage/' . $product->image) }}" width="80" class="img-thumbnail mb-2">
            @endif
        ...
        <div class="mb-3">
            ...
            <select name="category_id" class="form-control" onchange="loadCategoryFields(this.value)" required>
                ...
                @foreach($categories as $category)
                    <option value="{{ $category->id }}" data-guide="{{ $category->size_guide }}"
        ...
        <div class="mb-3">
            ...
            <textarea name="size_guide" class="form-control" rows="4">{{ $product->size_guide }}</textarea>
        </div>
...
<div class="modal fade" id="sizeGuideModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body" id="sizeGuideContent"></div>
        </div>
    </div>
</div>
...
document.addEventListener("DOMContentLoaded", function() {
    // Check if custom_fields is a string, then decode; otherwise use as-is
    const selectedFields = @json(is_string($product->custom_fields) ? json_decode($product->custom_fields, true) : ($product->custom_fields ?? []));

    loadCategoryFields("{{ $product->category_id }}", selectedFields);
});

function loadCategoryFields(categoryId, selectedFields = {}) {
    ...
    fetch(`/admin/category-fields/${categoryId}`)
        ...
        .then(data => {
            ...
            if (data.fields && data.fields.length > 0) {
                data.fields.forEach(field => {
                    ...
                    html += `</label><div class="custom-size-options">`;

                    field.options.forEach(option => {
                        const inputId = `${field.label}-${option}`.replace(/\s+/g, '-');
                        const isChecked = selectedFields[field.label]?.includes(option) ? 'checked' : '';

                        html += `
                            <input type="checkbox" class="btn-check" name="custom_fields[${field.label}][]" id="${inputId}" value="${option}" ${isChecked} autocomplete="off">
        ...
        .catch(error => console.error('Error fetching category fields:', error));
}
</script>
